...
def main(argv = sys.argv):
    ...
    # draw a rectangle around each location, using hardcoded values of size of
    # percents.
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            if not compare_with_previous(previous_frame, frame,
                ...
                # wait until stable again to look for difference between it and
                # previous one
                cur_stability = False
            else:
                cur_stability = True
            ...
            if cur_stability and not prev_stability:
                ...
                for idx, location in enumerate(extended_locations_found):
                    candidate = frame[
                        location[1]:location[1] + HEIGHT + (2 * BUFFER_SIZE),
                        location[0]:location[0] + WIDTH + (2 * BUFFER_SIZE)]
